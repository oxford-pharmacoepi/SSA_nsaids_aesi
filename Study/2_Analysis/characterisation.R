if (isTRUE(run_characterisation)) {
  ...
  # all nsaids ------
  # we characterise on all nsaids in one cohort
  # bind to the settings and remove rows where marker is gi hem or stroke broad
  # replace cohort_definition id with index_id (so it will run all nsaids together)
  cdm$nsaids_characteristics1 <- cdm$nsaids_aesi %>%
    left_join(
      settings(cdm$nsaids_aesi) %>%
        select(cohort_definition_id, index_id, index_name, marker_name),
      by = "cohort_definition_id",
    ...
    filter(!marker_name %in% c("gi_hemorrhage", "stroke_broad")) %>%
    filter(!index_name %in% c("acetaminophen", "aspirin")) %>%
    mutate(cohort_definition_id = index_id) %>%
    PatientProfiles::addDemographics(
      indexDate = "index_date",
      ageGroup = list(
    ...
    distinct(subject_id, index_id, .keep_all = TRUE) %>%  
    mutate(cohort_definition_id = 1, 
           index_id = 1,
           index_name = "all_nsaids") %>%
    compute(name = "nsaids_characteristics1", temporary = FALSE)
  
  
  # need to create new settings, attrition and codelist
  # settings
  new_settings <- cdm$nsaids_characteristics1 %>%
    distinct(cohort_definition_id, index_id, index_name) %>%  # all together!
    rename(cohort_name = index_name) %>%
    collect() %>%  # collect locally if needed
    tibble::as_tibble()
  ...
  # add new settings, attrition and codelist ref to new cohort table
  cdm$nsaids_characteristics1 <- omopgenerics::newCohortTable(
    table = cdm$nsaids_characteristics1,
    cohortSetRef = new_settings,
    cohortAttritionRef = new_attrition,
    cohortCodelistRef = new_codelist_ref,
    .softValidation = TRUE
  ...
  suppressWarnings({
    summarycharacteristics_all <- cdm$nsaids_characteristics1 %>%
      CohortCharacteristics::summariseCharacteristics(
        ageGroup = list(
          "18 to 49" = c(18, 49),
          "50 to 59" = c(50, 59),
  ...
  omopgenerics::exportSummarisedResult(
    summarycharacteristics_all,
    minCellCount = 5,
    path = here::here(characterisation_folder),
    fileName = paste0(cdmName(cdm), "_summary_characteristics_all_nsaids.csv")
  )
