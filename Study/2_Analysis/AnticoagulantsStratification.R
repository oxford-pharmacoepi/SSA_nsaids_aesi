...
if (!dir.exists(anticoag_strat_folder)) dir.create(anticoag_strat_folder, recursive = TRUE)

cli::cli_alert_success("- Running anticoagulants stratification analysis")
info(logger, "RUNNING ANTICOAGULANTS STRATIFICATION ANALYSIS")

cdm <- CohortSymmetry::generateSequenceCohortSet(
  cdm = cdm,
  name = "nsaids_aesi_no_anticoagulants",
  cohortDateRange = c(starting_date, ending_date),
  daysPriorObservation = 365,
  combinationWindow = c(0, 365),
  washoutWindow = 365,
  indexTable = "nsaids_no_anticoagulants",
  markerTable = "aesi"
)

cdm <- CohortSymmetry::generateSequenceCohortSet(
  cdm = cdm,
  name = "nsaids_aesi_prior_anticoagulants",
  cohortDateRange = c(starting_date, ending_date),
  daysPriorObservation = 365,
  combinationWindow = c(0, 365),
  washoutWindow = 365,
  indexTable = "nsaids_prior_anticoagulants",
  markerTable = "aesi"
)

#no prior anticoagulants
cli::cli_alert_success("- Running no prior anticoagulants analysis")
info(logger, "RUNNING NO PRIOR ANTICOAGULANTS ANALYSIS")

results_no_anticoag <- CohortSymmetry::summariseSequenceRatios(cohort = cdm$nsaids_aesi_no_anticoagulants)
...
cli::cli_alert_success("- Ran no prior anticoagulants analysis")
info(logger, "RAN NO PRIOR ANTICOAGULANTS ANALYSIS")

#prior anticoagulants
cli::cli_alert_success("- Running prior anticoagulants analysis")
info(logger, "RUNNING PRIOR ANTICOAGULANTS ANALYSIS")

results_anticoag <- CohortSymmetry::summariseSequenceRatios(cohort = cdm$nsaids_aesi_prior_anticoagulants)
